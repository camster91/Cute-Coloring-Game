// Custom Hooks
export { default as useMusic } from './useMusic';
export { default as useAmbientSounds } from './useAmbientSounds';
export { default as useColorHarmony, hexToHSL, hslToHex, generateColorHarmony } from './useColorHarmony';
export { default as useTouchGestures } from './useTouchGestures';
export { default as useToolState, brushTypes, shapeTools, symmetryModes } from './useToolState';
export { default as useCanvasState } from './useCanvasState';
export { default as useWellnessState, breathingPatterns, dailyPrompts, ambientSounds, formatTime } from './useWellnessState';
export { default as useLayerState } from './useLayerState';
